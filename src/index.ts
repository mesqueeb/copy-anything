 ...
 */
function cloneRef(value: any, clones: Map<any, any>, sources: any[], dests: any[]): any {
  ...
  while (sources.length) {
    ...
    if (Array.isArray(source)) {
      for (let i = 0; i < source.length; i++) {
        if (i in source) dest[i] = cloneRef(source[i], clones, sources, dests)
      }
      continue
    }
    ...
    if (nonenumerable) {
      for (const key of Object.getOwnPropertyNames(source)) {
        ...
        if (onlyProps && !onlyProps.includes(key as any)) continue
    ...
    for (const key of Object.getOwnPropertySymbols(source)) {
      if (onlyProps && !onlyProps.includes(key as any)) continue
      if (!nonenumerable && !Object.prototype.propertyIsEnumerable.call(source, key)) continue
      const newVal = cloneRef(source[key], clones, sources, dests)
      assignProp(dest, key, newVal, source)
    }
  }
